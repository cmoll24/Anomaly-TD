...
func create_navpath(start, _end):
	#print(start/32,end/32)
	var path = astargrid.get_point_path(start/32,Vector2(-2,-2))
	#print(path)
	return path
...
func _on_timer_timeout():
	spawn_unit(screen_size/2)#get_random_loc())#Vector2(screen_size.x/2,screen_size.y/2))
	timer.start()
...
func spawn_unit(pos):
	...
	enemy.start_navigation(create_navpath(enemy.position,enemy.target))
...
func place_turret(pos):
	...
	if towers.get_cell_tile_data(0, grid_pos2i) == null:
		...
		set_nav_collison(grid_pos)
	
	for child in unit_tree.get_children():
		if child is Enemy:
			child.start_navigation(create_navpath(child.position,child.target))
